...
const formatGitCommit = (event: NormalizedEvent): string => {
...
const formatGitPush = (event: NormalizedEvent): string => {
  const branch = event.content.branch ?? "unknown";
  const commitCount = event.content.tags?.length ?? 0;
  return `Branch: ${branch} - ${commitCount} commit(s)`;
};

const formatPlaneIssueUpdated = (event: NormalizedEvent): string => {
...
const formatPlaneIssueCreated = (event: NormalizedEvent): string => {
  const title = event.content.title ?? "Untitled";
  const taskId = event.content.taskId ?? "";
  return `Title: "${title}" - Task ID: ${taskId}`;
};

const formatDiscordMessage = (event: NormalizedEvent): string => {
...
const formatGenericEvent = (event: NormalizedEvent): string => {
  const time = formatTimestamp(event.eventTime);
  const parts: string[] = [
    `Source: ${event.source}.${event.sourceEventType}`,
    `Time: ${time}`,
  ];
  if (event.content.title) {
    parts.push(`Title: "${event.content.title}"`);
  }
  if (event.content.body) {
    parts.push(`Body: "${event.content.body}"`);
  }
...
const EVENT_FORMATTERS: Record<string, (event: NormalizedEvent) => string> = {
  "git.commit": formatGitCommit,
  "git.push": formatGitPush,
  "plane.issue_updated": formatPlaneIssueUpdated,
  "plane.issue_created": formatPlaneIssueCreated,
  "discord.message": formatDiscordMessage,
};

function formatEventDetails(event: NormalizedEvent): string {
  const key = `${event.source}.${event.sourceEventType}`;
  const formatter = EVENT_FORMATTERS[key] ?? formatGenericEvent;
...
export function formatWorkUnitsForSummary(
  ...
  lines.push("");
  lines.push(
    `### Work Units: ${workUnits.length} total (estimated ${totalMinutes} minutes)`
  );
  lines.push("");
  ...
  workUnits.forEach((wu, index) => {
    ...
    lines.push(
      `${index + 1}. **${wu.title}** (${minutes} min, confidence: ${confidence})`
    );
