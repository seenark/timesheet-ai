...
const makeRule = (
  overrides: Partial<AttributionRule> = {}
): AttributionRule => ({
  id: "rule-1",
  connectionId: "conn-1",
  organizationId: "org-1",
...
const makeMapping = (
  overrides: Partial<DirectMapping> = {}
): DirectMapping => ({
...
describe("attributeEvent", () => {
  ...
  test("prefers direct mapping over rules", () => {
    ...
    const mappings = [
      makeMapping({ externalIdentityId: "ext-id-1", canonicalUserId: "u1" }),
    ];
  ...
  test("applies branch-prefix rule when branch matches", () => {
    ...
    const rules = [
      makeRule({ pattern: "feature/", canonicalUserId: "u1", projectId: "p1" }),
    ];
  ...
  test("ignores rules for different connectionId", () => {
    const event = makeEvent({
      content: { branch: "feature/my-branch" },
      sourceRef: { connectionId: "conn-2", externalEventId: "x" },
    });
    const rules = [
      makeRule({
        connectionId: "conn-1",
  ...
  test("returns inferred attribution when event already has canonicalUserId and no rule matches", () => {
    const event = makeEvent({
      canonicalUserId: "u1",
      attribution: { identityConfidence: 0.8 },
    });
  ...
  test("returns projectId from event when no rule provides one", () => {
    const event = makeEvent({
      canonicalUserId: "u1",
      projectId: "p-event",
      attribution: { identityConfidence: 0.7 },
    });
    const result = attributeEvent(event, [], []);
    expect(result.attributionMethod).toBe("inferred");
