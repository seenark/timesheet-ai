...
export const createJobRun = async (
  ...
  if (!created) return err("Failed to create job run");
...
export const updateJobStatus = async (
  ...
  const updated = await db.merge(`${TABLE}:${id}`, updates) as unknown as JobRun | null;
  if (!updated) return err("Failed to update job run");
...
export const getPendingJobs = async (
  ...
  const [result] = await db.query(query, { jobType }) as unknown as [JobRun[]];
